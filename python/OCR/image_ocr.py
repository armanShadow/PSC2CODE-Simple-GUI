...
access_token = os.environ.get('VISION_API_KEY')
...
def google_ocr(video):
    #video_folder = video_name + '_' + video_hash

    images_folder = os.path.join(crop_dir, video)
    if not os.path.exists(images_folder):
        return
    
    out_folder = os.path.join(ocr_dir, video)
...
def main():
    ...
    for list_id, title in res:
        ...
        for video_hash, video_title in videos:
            ...
            google_ocr(video_title + '_' + video_hash)
