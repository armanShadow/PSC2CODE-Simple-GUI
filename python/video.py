...
class CVideo:
    def __init__(self, video_name, config={'eps1':2, 'eps2':1, 'min_samples':2, 'line_ratio':0.7}):
        ...
        for frame in frames:
            img_path = os.path.join(images_dir, video_name, "%d.png" % frame)
    ...
    def cluster_lines(self):
        ...
        for i in range(cluster_num):
            ...
            img_path = "%s/%s/%d.png" % (lines_dir, self.name, i)
            is_success, im_buf_arr = cv2.imencode(".png", blank_image2)
        ...
        img_path = "%s/%s/linemap.png" % (lines_dir, self.name)
        is_success, im_buf_arr = cv2.imencode(".png", blank_image)
        im_buf_arr.tofile(img_path)
    ...
    def adjust_lines(self):
        for cid in self.line_map:
            ...
            img_path = "%s/%s/%d_adjust.png" % (lines_dir, self.name, cid)
            is_success, im_buf_arr = cv2.imencode(".png", blank_image)
            im_buf_arr.tofile(img_path)
    ...
    def crop_rects(self):
        ...
        for cluster in clusters:
            ...
            for fidx, f in enumerate(cluster_frames):
                # img = self.images[f]
                if f not in frames:
                    img_path = os.path.join(images_dir, self.name, "%d.png" % f)
                    img = cv2.imdecode(np.fromfile(img_path, dtype=np.uint8),
                    cv2.IMREAD_UNCHANGED)
                ...
                img_path = "%s/%s/%s.png" % (crop_dir, self.name, img.name)
                is_success, im_buf_arr = cv2.imencode(".png", img.img[y1:y2, x1:x2])
                im_buf_arr.tofile(img_path)
